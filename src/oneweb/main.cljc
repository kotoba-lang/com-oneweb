(ns oneweb.main
  "Kotodama WASM entrypoint for the Oneweb clean-room actor (L4) — Clojure port.
  ...
  facts (`oneweb.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "oneweb")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"oneweb.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "oneweb-compat" :tier tier :entities entities} 200])
...
(defn start! [] :oneweb-compat/ready)
